...
from final_Neuron_Tab_vp14_with_Proofreading import create_local_colors , set_View, create_bounding_box
import time
...
def get_cgal_data_and_label(neuron_ID, decimation_ratio, clusters,smoothness):
    ...
    component_data = (ta3.ComponentAutoSegmentWhole() & [comp_dict]).fetch(as_dict=True)[0]
...
def get_highest_sdf_part(sdf_final_dict, labels_list,exclude_label=None):
    high_median_val = 0
    high_median = -1
    high_mean_val = 0
    high_mean = -1
    ...
    for x in my_list_keys:
        ...
        if sdf_final_dict[x]["median"] > high_median_val and my_list[x] > 1000:
            high_median = x
            high_median_val = sdf_final_dict[x]["median"]
        if sdf_final_dict[x]["mean"] > high_mean_val  and my_list[x] > 1000:
            high_mean = x
            high_mean_val = sdf_final_dict[x]["mean"]
        if sdf_final_dict[x]["max"] > high_max_val  and my_list[x] > 1000:
            high_max = x
...
"""Function that will rewrite PSEUDOCODE: 
Start the replacement label as “k” and neighbors_seen as [self], neighbors_to_check = []
Start loop that only breaks when replacement label is found
Get neighbors of current label using new function
Add all of neighbors (that aren’t part of the big labels) to the neighbors_seen list (if not already on there)
If one of them is part of big labels → use that as replacement label and break
Else, and to the neighbors to check (that aren’t already on there) and set the current label to the next label in the list of neighbors_to_check

Once found neighbor:
       7) if replacement label is soma, check if possible cillia and then label as cilia if fits, else just label as soma
Possible soma means neighbors or itself in possible threshold
      8) If not connected to soma or possible soma then rewrite all seen labels as the replacement lable
9) return the list
"""
...
def rewrite_label_vp2(labels_list,small_label,connections, mesh_Number,faces_raw,verts_raw,big_labels,soma_index,cilia_threshold=300):
    ...
    while replacement_label == "k":
        ...
        #so if a big label was not found in the neighbors -->
        #  add neighbors to the neighbors to check (that aren’t already on there) and set the current label to the next label in the list of neighbors_to_check
    ...
    for i,ll in enumerate(labels_list):
        if ll in neighbors_list:
            ...
            for vert in verts_from_faces:
                ...
                if real_vert.co[1] < min_max[ll][0]:
                    min_max[ll][0] = real_vert.co[1]
                if real_vert.co[1] > min_max[ll][1]:
                    min_max[ll][1] = real_vert.co[1]
    return min_max
...
if __name__ == "__main__":
    ...
    try:
        ...
        @schema
        class AnnotationLast100(dj.Computed):
            definition = """
            # creates the labels for the mesh table
            -> ta3.ComponentAutoSegmentWhole
            ...
            def make(self, key):  
                ...
                highest_vals= get_highest_sdf_part(sdf_final_dict, labels_list )
                #print(highest_vals)
                ...
                whole_neuron_labels = classify_whole_neuron(possible_Apical,soma_index,connections,mesh_Number,sdf_final_dict,threshold=700)
...
AnnotationLast100.populate(reserve_jobs=True)
