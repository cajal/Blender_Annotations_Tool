...
def filter_verts_and_faces(key,verts,faces):
    ...
    verts_label, triangles_label = (ta3.Compartment.Component & key).fetch('vertex_indices','triangle_indices')
...
def load_Neuron_automatic_spine(key):
    ...
    if ID not in whole_neuron_dicts:
        whole_neuron_dicts[ID] = (ta3.CleansedMesh & 'decimation_ratio=0.35' & dict(segment_id=ID)).fetch1()
...
def get_cgal_data_and_label(key,ob_name):
    ...
    component_data = (ta3.ComponentAutoSegment() & key).fetch(as_dict=True)
    if component_data == []:
        return [], []
    else:
